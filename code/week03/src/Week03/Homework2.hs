...
mkValidator pkh deadline _ ctx = isSignedByBeneficiary && isInDeadline
    where
        info :: TxInfo
        info = scriptContextTxInfo ctx

        isSignedByBeneficiary :: Bool
        isSignedByBeneficiary = txSignedBy info $ unPaymentPubKeyHash pkh

        isInDeadline :: Bool
        isInDeadline = contains (from deadline) $ txInfoValidRange info
